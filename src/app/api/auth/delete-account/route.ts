...
export async function POST() {
    ...
    try {
        ...
        if (hostedMeetupIds.length > 0) {
            // Delete Messages in hosted meetups
            const { error: mmError } = await adminSupabase
                .from('meetup_messages')
                .delete()
                .in('meetup_id', hostedMeetupIds);
            checkError('Delete Messages in Hosted Meetups', mmError);
        ...
        const { error: meetupMsgError } = await adminSupabase
            .from('meetup_messages')
            .delete()
            .eq('sender_id', userId);
        checkError('Delete Meetup Messages (Sent)', meetupMsgError);
